...
import { CATEGORY_NEWS, FORMATE_DATE_TIME, FORMAT_DATE, STAFF_PATH, STUDENT_PATH } from "utils/constant";
...
function TablePengumumanSiswa({ fetcher }: Props) {
    const columns: ColumnsType<Pengumuman> = [
        ...
        },
        {
            ...
            fixed: "right",
            render: (_, record) => (
                <Link to={`${STUDENT_PATH.pengumuman.index}/${record.id}`}>
                    <Button>Lihat</Button>
                </Link>
            ),
        },
...
export default TablePengumumanSiswa;
