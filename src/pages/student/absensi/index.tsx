...
import { ABSEN_STATUS, CLASSES, COUNT_ABSENT, MONTHS, TOTAL_DAY } from "utils/constant";
...
function MonthHistory({ cls }: MonthHistoryProps) {
    const { state } = React.useContext(UserContext);

    const getMyAttendanceQuery = useQuery(["getMyAttendance", cls], async () => {
        return (await getMyAttendance({ student_id: state?.user?.id })).data as any;
    });
